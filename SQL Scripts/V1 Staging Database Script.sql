...
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'V1StagingPlatts')
BEGIN
	DROP DATABASE V1StagingPlatts;
END

--Create the database.
CREATE DATABASE V1StagingPlatts;
GO

--Set the recovery model.
--ALTER DATABASE V1Staging SET RECOVERY SIMPLE WITH NO_WAIT;
--GO

--Change data file initial size and file growth values
ALTER DATABASE V1StagingPlatts MODIFY FILE 
(NAME = N'V1StagingPlatts', SIZE = 100MB , FILEGROWTH = 25MB);
GO

--Change log file initial size and file growth values
ALTER DATABASE V1StagingPlatts MODIFY FILE 
(NAME = N'V1StagingPlatts_log', SIZE = 100MB , FILEGROWTH = 25MB);
GO
